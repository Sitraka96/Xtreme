...
app.use((req, res, next) => {
  res.setHeader('Access-Control-Allow-Origin', 'http://localhost:8080', 'http://localhost:8100', 'http://localhost:4200'); // Autorise toutes les origines (à remplacer par votre domaine spécifique si nécessaire)
  res.
 
setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE'); // Autorise les méthodes HTTP spécifiées
  res.setHeader('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept, Authorization'); // Autorise les en-têtes spécifiés
  next();
});
