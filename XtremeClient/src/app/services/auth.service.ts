...
export class AuthService {
  ...
  checkToken()
  {
    this.storage.get(TOKEN_KEY).then((token: string | undefined)=>{
      if(token)
      {
  ...
  inscription(credentials:any) {
		return this.http.post(`${this.url}/inscription`, credentials, httpOptions).pipe(
			catchError((e) => {
  ...
  connexion(credentials:any) {
	...
	logout(): Observable<void> {
  	...
  	getSpecialData() {
		return this.http.get(`${this.url}/clients`, httpOptions).pipe(
			catchError((e) => {
				let status = e.status;
				if (status === 401) {
