...
exports.enregistrementNouveauClient = async(req, res) => {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, Authorization");
    ...
    try {
      ...
      /*if (!photo_profil) {
          photo_profil = 'public\\profil.png'
      }*/

      /*if(!email_clientRegex.test(email_client)) {
          res.status(400).json({msg: 'email_client non valide !'});    
          return;
      }*/

      /*if(mot_de_passe_client !== confirmationmot_de_passe_client) {
          res.status(400).json({msg: 'Mot de passe et sa confirmation différents !'});    
          return;
      }*/

      const client = await Clients.findOne({
        where:{
          [Op.or]:[{ email_client }, { id_client: email_client }] 
      }
      });

      if(client) {
          if(Clients.email_client === email_client) res.status(400).json({ msg: 'Cet email existe déja !' });
        
      }
      else {
          
          /*var tabNomFichier = ent.encode(parse.photo_profil_path).split("/");
          var fileName= tabNomFichier[tabNomFichier.length -1];

          var sary = req.body.photo_profil
      
          const base64data = sary.replace(/^data:.*,/, '');
          fs.writeFile("public/images/Clientss/" + fileName, base64data, 'base64', (err) => {
            if (err) {
